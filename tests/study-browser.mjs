...
for(const viewport of [{width:1400,height:1000},{width:390,height:844}]){
 ...
 for(const t of tasks)for(const p of t.parts){
  ...
  assert.equal(await page.locator('#hint, #hints, #work-note, textarea').count(),0);
  assert.ok(await page.locator('.exam-answer').isVisible());
  assert.equal(await page.locator('.solution > .exam-answer + details[data-explanation]').count(),1);
  if(viewport.width===1400)assert.equal(await page.locator('[data-explanation]').evaluate(e=>e.open),false);
  await page.locator('[data-explanation]').evaluate(e=>e.open=true);
...
// Reload restores inputs and disclosure state; old notes remain exportable. First errors prevent a false independence rating.
await page.setViewportSize({width:1400,height:1000});await page.goto(base+'#/training/original-tlu/netz');
await page.evaluate(()=>{const key='ann_exam_focus_v1',s=JSON.parse(localStorage.getItem(key));s.drafts['original-tlu'].parts.netz.note='Eine erhaltene Testnotiz.';localStorage.setItem(key,JSON.stringify(s));});await page.reload({waitUntil:'networkidle'});await page.locator('#numbers').evaluate(e=>e.open=true);await page.locator('#answer-h1').fill('(0; 1; 4)');await page.locator('#answer-form button').click();assert.ok(await page.locator('[data-rating="independent"]').isDisabled());
await page.reload({waitUntil:'networkidle'});assert.equal(await page.locator('#work-note').count(),0);assert.equal(await page.locator('[data-explanation]').evaluate(e=>e.open),true);assert.equal(await page.locator('#answer-h1').inputValue(),'(0; 1; 4)');assert.equal(await page.locator('#reveal').getAttribute('aria-expanded'),'true');assert.ok(await page.locator('[data-rating="independent"]').isDisabled());
// Every legacy route resolves to useful new content, without restoring a standalone recall trainer.
for(const path of ['klausur','abfragen','task1','task2','task3','task4','task5','task6','cheatsheet','aufgaben','regression','klassifikation','modellwahl','theorie','deeplearning']){await page.goto(base+'#/'+path);await page.waitForTimeout(50);assert.ok((await page.locator('h1').innerText()).length>0);assert.equal(await page.locator('nav[aria-label="Hauptnavigation"] a').count(),4);}
// One complete paper, all questions/parts visible before reveal, then all solutions present.
await page.goto(base+'#/gesamt');await page.locator('[data-paper="original"]').click();assert.equal(await page.locator('.paper-task').count(),5);assert.equal(await page.locator('[data-block]').count(),90);assert.equal(await page.locator('[data-paper-note],textarea').count(),0);
const block=await page.evaluate(()=>{const s=JSON.parse(localStorage.getItem('ann_exam_focus_v1'));return NNMC.concepts.filter(c=>c.core).map(c=>NNMC.question(c,s.paper.turn));});for(const q of block)await page.locator(`input[data-block="${q.id}"][value="${q.correct}"]`).check();
await page.reload({waitUntil:'networkidle'});await page.locator('#paper-finish').click();assert.equal(await page.locator('.paper-solution').count(),5);assert.match(await page.locator('#app').innerText(),/30 \/ 30 Punkte/);assert.equal(await page.locator('.exam-answer').count(),9);assert.equal(await page.locator('.katex-error').count(),0);
...
await p.goto(base+'#/training/transfer-hopfield/graph');await p.locator('#reveal').click();await p.locator('.exam-answer .hopgraph').scrollIntoViewIfNeeded();await p.screenshot({path:'tmp/nn-hopfield-viewport.png'});await p.locator('.exam-answer [data-hop-focus]').selectOption('0');assert.ok(await p.locator('.exam-answer .hopgraph').evaluate(e=>e.classList.contains('focused')));
...
assert.deepEqual(errors,[]);assert.deepEqual(broken,[]);console.log(`PASS: ${checked} rendered task parts at desktop/mobile; formulas, fields, persistence, 15 old routes, compact/detailed solutions, complete paper, export/import and source links.`);await browser.close();
